...
# tkinterdnd2 임포트 (없으면 기본 tkinter로 폴백)
try:
...
log_filename = os.path.join(LOG_DIR, f"ocr_{datetime.now():%Y%m%d}.log")
...
class PDFOCRApp:
    ...
    def __init__(self):
        ...
        self.file_queue: list = []  # 처리 대기 파일 목록
        self.is_processing = False
        self.engine: KoEnOCREngine | None = None
    ...
    def _build_ui(self):
        ...
        self.gpu_label = ttk.Label(btn_frame, text="⏳ 엔진 로딩 중...")
        self.gpu_label.pack(side="right")

        # ── 진행률 영역 ──
        progress_frame = ttk.LabelFrame(
            self.root, text="진행 상황", padding=10
        )
        ...
        self.file_progress_bar.pack(side="left", fill="x", expand=True, padx=(5, 0))
        ...
        self.page_progress_bar.pack(side="left", fill="x", expand=True, padx=(5, 0))

        # ── 상태 라벨 ──
        self.status_label = ttk.Label(
            self.root, text="대기 중", anchor="center", font=("맑은 고딕", 10)
        )
    ...
    def _warmup_engine(self):
        ...
        def _do_warmup():
            try:
                self.engine = KoEnOCREngine()
                self.engine.warmup()
                gpu_text = "🟢 GPU" if self.engine.use_gpu else "🟡 CPU"
                self.msg_queue.put(("engine_ready", gpu_text))
            except Exception as e:
                logger.error(f"엔진 워밍업 실패: {e}")
    ...
    def _handle_message(self, msg_type: str, data):
        """큐에서 받은 메시지에 따라 GUI 업데이트"""
        if msg_type == "engine_ready":
            self.engine_ready = True
            self.gpu_label.config(text=f"{data} 모드 준비 완료")
            logger.info(f"OCR 엔진 준비 완료: {data}")

        elif msg_type == "engine_error":
            self.gpu_label.config(text="❌ 엔진 로드 실패")
            ...
            self.page_progress_var.set(percent)
            self.status_label.config(text=f"{current}/{total}쪽 처리 중... {percent}%")
    ...
    def _start_processing(self, pdf_files: list):
        ...
        if self.is_processing:
            messagebox.showwarning("처리 중", "이미 파일을 처리하고 있습니다. 완료 후 다시 시도하세요.")
            return
    ...
    def _process_files(self, pdf_files: list):
        """여러 PDF 파일을 순차적으로 OCR 처리"""
        total_files = len(pdf_files)

        for file_idx, pdf_path in enumerate(pdf_files):
            filename = os.path.basename(pdf_path)
            self.msg_queue.put(("status", f"📄 {filename} 처리 중... ({file_idx + 1}/{total_files})"))
            self.msg_queue.put(("file_progress", (file_idx, total_files)))
    ...
    def _process_single_pdf(self, pdf_path: str, filename: str):
        ...
        for page_idx, pix in enumerate(pixmaps):
            self.msg_queue.put(("page_progress", (page_idx + 1, total_pages)))
        ...
        build_searchable_pdf(doc, ocr_results_per_page, output_path, dpi=DPI)

        output_filename = os.path.basename(output_path)
        self.msg_queue.put(("file_done", output_filename))
